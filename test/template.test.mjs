import { Template } from "../dist/ftl.mjs";
...
describe('Template', () => {
    ...
    it('can iterate with *-each', () => {
        let data = [1, 2];
        let template = Template.fromHtml('<div data-tpl-each="self">{{self}}</div>', modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '<div>1</div><div>2</div>');
    });
    it('can skip rendering with *-if', () => {
        let data = {};
        let template = Template.fromHtml('<div data-tpl-if="false">{{v}}</div>', modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '');
    });
    it('can render with *-if', () => {
        let data = {a: 1};
        let template = Template.fromHtml('<div data-tpl-if="true">{{a}}</div>', modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '<div>1</div>');
    });
    it('can render html from attribute', () => {
        let data = {a: '<h1>test</h1>'};
        let template = Template.fromHtml('<div data-tpl-html="a">nope</div>', modules);
        let rendered = template.render(data);
    ...
    it('can render text from attribute', () => {
        let data = {a: '<h1>test</h1>'};
        let template = Template.fromHtml('<div data-tpl-text="a">nope</div>', modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '<div>&lt;h1&gt;test&lt;/h1&gt;</div>');
    });
    ...
    it('can render text from a text node', () => {
        let data = {a: "<>"};
        let template = Template.fromHtml("<div>b{{a}}d</div>", modules);
        let rendered = template.render(data);
    ...
    it('can evaluate a data-* attribute', () => {
        let data = {a: 1, b: 2};
        let template = Template.fromHtml('<div data-tpl-former="a" data-tpl-latter="b">content</div>', modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '<div former="1" latter="2">content</div>');
    });
    it('can *-remove-tag', () => {
        let data = [1, 2, 3, 4];
        let template = Template.fromHtml('<div data-tpl-remove="tag">123</div>', modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '123');
    });
    it('removing tag does not cause double evaluation', () => {
        let data = {a: "{{'1'}}"};
        let template = Template.fromHtml('<div data-tpl-remove="tag">{{a}}</div>', modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), "{{'1'}}");
    });    
    it('can *-remove-tag from *-each', () => {
        let data = [1, 2, 3, 4];
        let template = Template.fromHtml('<div data-tpl-each="self" data-tpl-remove="tag">{{ self }}</div>', modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '1234');
    });
    it('can *-remove-tag from *-if', () => {
        let data = {};
        let template = Template.fromHtml('<div data-tpl-if="true" data-tpl-remove="tag">{{ 1 }}</div>', modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '1');
    });
    it('can evaluate nested tags (each -> if)', () => {
        let data = [1, 2, 3, 4];
        let template = Template.fromHtml('<div data-tpl-each="self"><span data-tpl-if="#math:isEven(self)">{{ self }}</span></div>', modules);
        let rendered = template.render(data);
    ...
    it('can evaluate nested tags (each -> if) removing tags', () => {
        let data = [1, 2, 3, 4];
        let template = Template.fromHtml('<div data-tpl-each="self" data-tpl-remove="tag"><span data-tpl-if="#math:isEven(self)" data-tpl-remove="tag">{{ self }}</span></div>', modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '24');
    });
    it('can evaluate nested tags (if -> each)', () => {
        let data = [1, 2, 3, 4];
        let template = Template.fromHtml('<div data-tpl-if="#math:isEven(2)"><span data-tpl-each="self">{{ self }}</span></div>', modules);
        let rendered = template.render(data);
    ...
    it('can evaluate nested tags (if -> each) removing tags', () => {
        let data = [1, 2, 3, 4];
        let template = Template.fromHtml('<div data-tpl-if="#math:isEven(2)" data-tpl-remove="tag"><span data-tpl-each="self" data-tpl-remove="tag">{{ self }}</span></div>', modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '1234');
    });
    it('inner text node is not reevaluated when generated by tpl-text', () => {
        let data = {a: 1};
        let template = Template.fromHtml(`<div data-tpl-text="'{{a}}'"></div>`, modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '<div>{{a}}</div>');
    });
    it('inner html node is not reevaluated when generated by tpl-html', () => {
        let data = {a: 1};
        let template = Template.fromHtml(`<div data-tpl-html="'{{a}}'"></div>`, modules);
        let rendered = template.render(data);
        assert.strictEqual(toHtml(rendered), '<div>{{a}}</div>');
    });    
